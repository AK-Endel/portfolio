                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                RSWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Liik: '..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Väetis: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Seisund: ~b~'..plantData["plantStats"]["health"]..'%')
                            else
                                RSWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"] + 0.2, 'Taim on valmis! ~g~E~w~ - Korja saaki')
                                RSWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Liik: ~g~'..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Väetis: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Seisund: ~b~'..plantData["plantStats"]["health"]..'%')
                                if IsControlJustPressed(0, RSWeed.Keys["E"]) then
                                    RSCore.Functions.Progressbar("remove_weed_plant", "Korjamine..", 8000, false, true, {
                                        ...
                                        ClearPedTasks(ped)
                                        RSCore.Functions.Notify("Katkesta..", "error")
                                    end)
                                end
                            end
                        elseif plantData["plantStats"]["health"] == 0 then
                            RSWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Taim on surnud, ~r~E~w~ - Korista taim')
                            if IsControlJustPressed(0, RSWeed.Keys["E"]) then
                                RSCore.Functions.Progressbar("remove_weed_plant", "Taime koristamine..", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    RSCore.Functions.Notify("Katkestatud..", "error")
                                end)
                            end
                        end
                    end
...
AddEventHandler('rs-weed:client:placePlant', function(type, item)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            RSCore.Functions.Progressbar("plant_weed_plant", "Taime istutamine..", 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
            ...
            RSCore.Functions.Notify('Siin pole taimel ruumi..', 'error', 3500)
        end
    else
        RSCore.Functions.Notify('Siin pole piisavalt ohutu..', 'error', 3500)
    end
end)
        ...
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    RSCore.Functions.Notify('Taimel pole hetkel väetist vaja..', 'error', 3500)
                else
                    RSCore.Functions.Progressbar("plant_weed_plant", "Taime väetamine..", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                ...
                RSCore.Functions.Notify("Siin pole taimel ruumi..", "error")
            end
        else
            RSCore.Functions.Notify("Siin pole taimel ruumi..", "error")
        end
