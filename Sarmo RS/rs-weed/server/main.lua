...
RSCore.Commands.Add("istutataim", "", {}, false, function(source, args)
    local src           = source
            ...
            if house ~= nil then 
                RSCore.Functions.ExecuteSql(false, "SELECT * FROM `house_plants` WHERE plantid = '"..plantId.."' AND building = '"..house.."'", function(result)
                    if result[1] ~= nil then
                        ...
                        TriggerClientEvent('RSCore:Notify', src, 'Taim on korjatud', 'success', 3500)
                        TriggerClientEvent('rs-weed:client:refreshHousePlants', -1, house)
                    else
                        TriggerClientEvent('RSCore:Notify', src, 'Seda taime pole enam olemas', 'error', 3500)
                    end
                end)
            else
                TriggerClientEvent('RSCore:Notify', src, 'Sa pole majas?', 'error', 3500)
            end
        else
            TriggerClientEvent('RSCore:Notify', src, 'Sul pole piisavalt gripkotte', 'error', 3500)
        end
    else
        TriggerClientEvent('RSCore:Notify', src, 'Sul pole gripkotti', 'error', 3500)
    end
end)
