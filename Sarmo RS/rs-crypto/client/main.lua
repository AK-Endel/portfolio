				...
				if dist < 15 then
					...
					if dist < 1.5 then
						if not Crypto.Exchange.RebootInfo.state then
							DrawText3Ds(Crypto.Exchange.coords.x, Crypto.Exchange.coords.y, Crypto.Exchange.coords.z, '~g~E~w~ - Sisesta USB')
							...
							if IsControlJustPressed(0, Keys["E"]) then
								RSCore.Functions.TriggerCallback('rs-crypto:server:HasSticky', function(HasItem)
									if HasItem then
										RSCore.Functions.Progressbar("deliver_reycle_package", "USB dekrüpteerimine..", 20000, false, true, {
										...
										RSCore.Functions.Notify('Sul ei ole krüpto USBd..', 'error')
									end
								end)
							end
						else
							DrawText3Ds(Crypto.Exchange.coords.x, Crypto.Exchange.coords.y, Crypto.Exchange.coords.z, 'Süsteemi taaskäivitamine - '..Crypto.Exchange.RebootInfo.percentage..'%')
						end
					else
